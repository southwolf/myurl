...
class XMLHelper
    ...
    def ExportToExcel(outTables = nil, dictFactory = nil, session = nil)        
        ...
        excel.ExportToExcel(outTables || @tables, dictFactory, session)
	end
...
#            result << "\t\t</form>\n"
    ...
    def MS_ReadFromXMLString(src)
        ...
        begin
          ...
          YtLog.info err.backtrace
          YtLog.info "create Msxml2.DOMDocument.6.0 object Faild!" 
        end        
        xml.preserveWhiteSpace = true
        xml.loadXML(src)
        MS_ReadFromXML(xml)
    ...
    private
        def MS_ReadFromXML(doc)
            ...
            begin
              ...
              YtLog.info err.backtrace
              YtLog.info "MS_ReadFromXML Failed!"
          end
        end
        ...
        def AddAnEditHTMLCell2(table,row, col, dicFactory, readonly=false, record=nil, float=false, currency=nil)
            ...
            if !cell.IsStore() || table.IsEmptyRow(row) || table.IsEmptyCol(col)
                ...
                if cell.GetInputType() == CCell::ItComboBox
                    ...
                    result << "\t\t\t\t<td align='left'  #{colspanstring} #{rowspanstring}  #{"valign=top" if rowheight>7} class='style#{cell.GetStyle().GetID()}' expression='#{table.GetTableID()}.#{table.GetCellDBFieldName(row, col)}' #{"style='background-color=#{backcolor}'" if cell.IsReadOnly() || float}>"
                    if !readonly    #可编辑
