...
class XMLHelper
    ...
    def ExportToExcel(outTables = nil, dictFactory = nil)        
        ...
        excel.ExportToExcel(outTables,dictFactory)
	end
    ...
    def TableToEditHTML(table, dicFactory, *args)
        return if !table
        ...
        if !onlytabletag
            ...
            result << StyleToHTML(table)
            result << "\t</head>\n"
            result << "\t\t<body>\n"
            #p readonly
            if !readonly
                result << EncodeUtil.change("GB2312", "UTF-8", '<script >
...
#           end
        ...
        if !readonly
        #    result = EncodeUtil.change("UTF-8", "GB2312", result) if encode == "utf8"
        ...
        result << ".yotopTable
{
    ...
    private
        ...
        def AddAnEditHTMLCell2(table,row, col, dicFactory, readonly=false, record=nil, float=false, currency=nil)
            ...
            fieldId = "#{table.GetTableID()}_#{cell.GetName()}".downcase()
            fieldName = "#{table.GetTableID()}[#{cell.GetName()}]".downcase()
            
            if !cell.IsStore() || table.IsEmptyRow(row) || table.IsEmptyCol(col)
                result << "\t\t\t\t<td align='#{align}' #{colspanstring} #{rowspanstring} class = 'style#{cell.GetStyle().GetID()}' style='width:#{cs(width)}mm; height:#{cs(height)}mm; background: buttonface;'>#{cell.GetText().gsub(" ", "&nbsp;")}</td>\n"
            else
                ...
                if cell.GetInputType() == CCell::ItComboBox
                    ...
                    if dict
                        result << "\t\t\t\t<td align='left' #{colspanstring} #{rowspanstring} class = 'style#{cell.GetStyle().GetID()}' expression='#{table.GetTableID()}.#{table.GetCellDBFieldName(row, col)}' #{'style=background-color=' + backcolor if cell.IsReadOnly()|| float}>\n"
                        if !readonly 
                            result << "<select #{'disabled="disabled"' if cell.IsReadOnly()} name='#{fieldName}' id='#{fieldId}' style='BORDER-BOTTOM: solid 1px; BORDER-LEFT: dashed 0px; BORDER-RIGHT: dashed 0px; BORDER-TOP: dashed 0px; width:#{colwidth-1}mm; height:#{rowheight-1}mm; background-color:#{backcolor}'>\n"
                            if dict
                            ...
                            if record
                                result << EncodeUtil.change("GB2312", "UTF-8", dict.GetItemName(EncodeUtil.change("GB2312", "UTF-8", record["#{table.GetCellDBFieldName(row, col)}"].to_s)))  #外部传入了记录
                            else
                    ...
                    result << "\t\t\t\t<td align='left' #{colspanstring} #{rowspanstring} class='style#{cell.GetStyle().GetID()}' expression='#{table.GetTableID()}.#{table.GetCellDBFieldName(row, col)}' #{'style=\'width:'+cs(table.GetColWidth(col)).to_s + 'mm; height:'+cs(table.GetRowHeight(row)).to_s + 'mm;\'' if cell.GetCoveredScale.cx == 1 } #{'style=background-color=' + backcolor if cell.IsReadOnly() || float}>"
                    if !readonly    #可编辑
                        result <<"<#{typetext} type=text onchange='calc()'  #{'readonly' if cell.IsReadOnly} name='#{fieldName}' id='#{fieldId}' style='width:#{cs(table.GetColWidth(col))}mm; height:#{cs(table.GetRowHeight(row))}mm; BORDER-BOTTOM: solid 1px; BORDER-LEFT: dashed 0px; BORDER-RIGHT: dashed 0px; BORDER-TOP: dashed 0px; width:#{colwidth-1}mm; height:#{rowheight-1}mm; background-color:#{backcolor}'></#{typetext}>"
                    else            #只读
                        #考虑精度
