...
oCalendarChs.Init();
        ...
        Integer(0).upto(table.GetColumnCount()-1) do |col|
          ...
          result << "\t\t\t<col width=#{cs(table.GetColWidth(col))}mm></col>\n"
        end
        ...
        result << ".yotopTable
{
    ...
    table-layout: fixed;
        ...
        result <<"\t\t\t</style>\n"
        EncodeUtil.change(encoding, 'GB2312', result)
    end
        ...
        def AddAnEditHTMLCell2(table,row, col, dicFactory, readonly=false, record=nil, float=false, currency=nil)
            ...
            if !cell.IsStore() || table.IsEmptyRow(row) || table.IsEmptyCol(col)
                ...
                if cell.GetInputType() == CCell::ItComboBox
                    ...
                    if dict
                        style = "style='width:#{cs(width)}mm;height:#{cs(height)}mm' "
                        style += "background-color=#{backcolor}" if cell.IsReadOnly()|| float
                        result << "\t\t\t\t<td align='left' #{colspanstring} #{rowspanstring} class = 'style#{cell.GetStyle().GetID()}' expression='#{table.GetTableID()}.#{table.GetCellDBFieldName(row, col)}' #{style}>\n"
                        if !readonly 
                    ...
                    result << "\t\t\t\t<td align='left' #{colspanstring} #{rowspanstring} class='style#{cell.GetStyle().GetID()}' expression='#{table.GetTableID()}.#{table.GetCellDBFieldName(row, col)}' width=#{cs(table.GetColWidth(col)).to_s}mm height=#{cs(table.GetRowHeight(row)).to_s}mm #{'style=background-color='+backcolor if cell.IsReadOnly() || float}><input readonly  #{'onclick="getDateString(this,oCalendarChs)"' if !cell.IsReadOnly()} type=text name='#{fieldName}' id='#{fieldId}' style='BORDER-BOTTOM: solid 1px; BORDER-LEFT: dashed 0px; BORDER-RIGHT: dashed 0px; BORDER-TOP: dashed 0px; width:#{colwidth-1}mm; height:#{rowheight-1}mm; background-color:#{backcolor}'></input></td>\n"
                else 
                    typetext = "input"
                    typetext = "textarea" if rowheight > 7    
                    
                    tsz = "width:#{cs(width)}mm;height:#{cs(height)}mm" if cell.GetCoveredScale.cx == 1
                    style = "style='#{tsz}' "     
                    style += "background-color='#{backcolor}'" if cell.IsReadOnly() || float
                    result << "\t\t\t\t<td align='left' valign='middle' #{colspanstring} #{rowspanstring} class='style#{cell.GetStyle().GetID()}' expression='#{table.GetTableID()}.#{table.GetCellDBFieldName(row, col)}' #{style}>"
                    if !readonly    #可编辑
                        result <<"<#{typetext} type=text  #{'readonly' if cell.IsReadOnly} name='#{fieldName}' id='#{fieldId}' style='width:100%; height:90%; BORDER-BOTTOM: solid 1px; BORDER-LEFT: dashed 0px; BORDER-RIGHT: dashed 0px; BORDER-TOP: dashed 0px; background-color:#{backcolor}'></#{typetext}>"
                    else            #只读
                        #考虑精度
