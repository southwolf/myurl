...
################################################################ 
  ...
  def create
   begin
    ...
    #先创建任务
    @task = Task.new
    @task.strid = params["task.id"]
    @task.name = params["task.name"]
    @task.version = 1
    @task.datecreated = Time.new
    @task.content = EncodeUtil.change("UTF-8", "GB2312", xmlstr)
    @task.content = @task.view
    @task.save
    YtLog.info "任务记录保存"
    ...
    for table in tables
      ...
      sql = "create table ytapl_#{@task.strid}_#{table.GetTableID()} (unitid varchar(30) ";
      ...
      Integer(0).upto(table.GetRowCount()-1) do |row|
        ...
        if table.IsFloatTemplRow(row)
          floatsql = "create table ytapl_#{@task.strid}_#{table.GetTableID()}_#{table.PhyRowToLogicRow(row+1)} (unitid varchar(30), tasktimeid int, float_id int ";
          floatsql += ", primary key (unitid, tasktimeid, float_id))"
          conn.execute(floatsql)        
          Integer(0).upto(table.GetColumnCount()-1) do |col|  
            add_field("ytapl_#{@task.strid}_#{table.GetTableID()}_#{table.PhyRowToLogicRow(row+1)}", table, row, col)  
          end        
        end        
        
        Integer(0).upto(table.GetColumnCount()-1) do |col|  
          next if !table.GetCell(row, col).IsEffective()          
          add_field("ytapl_#{@task.strid}_#{table.GetTableID()}", table, row, col)  
        end
      end
    end
  ...
  def destroy
    ...
    for table in meta.helper.tables
      Integer(0).upto(table.GetRowCount()-1) do |row|
        if table.IsFloatTemplRow(row)
          #conn.execute("drop table if exists ytapl_#{task.strid}_#{table.GetTableID()}_#{table.PhyRowToLogicRow(row+1)} ")
          begin
            conn.drop_table "ytapl_#{task.strid}_#{table.GetTableID()}_#{table.PhyRowToLogicRow(row+1)}"
          rescue
          end
        end
      end
      #conn.execute("drop table if exists ytapl_#{task.strid}_#{table.GetTableID()} ")
      begin
        conn.drop_table "ytapl_#{task.strid}_#{table.GetTableID()}"
      rescue
      end
    end
...
private
  ...
  def add_field(tablename, table, row, col)
          conn = ActiveRecord::Base.connection
          cell = table.GetCell(row, col)          
          return if table.GetCellDBFieldName(row, col).strip.size == 0
          if cell.GetDataType() == 1 #numeric
              conn.add_column(tablename, table.GetCellDBFieldName(row, col), "decimal(20,5)")
          elsif cell.GetDataType() == 3 #date
              conn.add_column(tablename, table.GetCellDBFieldName(row, col), :datetime)
          else                          #text
            if cell.IsCheckWidth()
              conn.add_column(tablename, table.GetCellDBFieldName(row, col), :string, {:limit=>cell.GetTextWidth})
            else
              conn.add_column(tablename, table.GetCellDBFieldName(row, col), :string, {:limit=>100})
            end            
          end
  end
end
