class Notice1Controller < ApplicationController
  ...
  def list
    #@ytwg_news_pages, @ytwg_news = paginate :notice, :per_page => 20, :order=>"id desc"
     @ytwg_news = Notice1.paginate :page=>params[:page], :per_page => 10, :order=>"id desc"
  end

  def show
    @ytwg_news = Notice1.find(params[:id])
    @ytwg_news.users << session[:user] rescue nil
  end

  def new
    @ytwg_news = Notice1.new
  end

  def create
    @ytwg_news = Notice1.new(params[:ytwg_news])
  ...
  def destroy
    Notice1.find(params[:id]).destroy
    redirect_to :action => 'list'
    flash[:notice] = '删除通知成功'
  end
  
  def read
    @notice = Notice1.find(params[:id])
  end
end
