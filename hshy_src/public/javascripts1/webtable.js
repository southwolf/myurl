...
{	
    ...
    if (Prototype.Browser.IE)
      ...
      tree_element.cell.textContent = text;   
...
function submit_webtable(objectname, form)
{
    ...
    function(node)
    {
        ...
        {
            var n = $(node.getAttribute('expression').replace(/(\w+)\.(\w+)/, "$1_$2"))   //先删除
            if (n)        //如果有就先删除
                n.removeNode(true);   

            var oInput =document.createElement("input");
            oInput.type = "hidden";
            oInput.id = node.getAttribute('expression').replace(/(\w+)\.(\w+)/, "$1_$2");
            oInput.name = node.getAttribute('expression').replace(/(\w+)\.(\w+)/, "$1[$2]");
            if (node.code_value)
            {
...
function createInput(cell, value)
{
    ...
    {
    	var div = $(cell.readAttribute('tree'));
    ...
    else
    {
        if (cell.offsetHeight < 50)
          cell.innerHTML = "<input type='text' class='required' style='height:100%; width:100%;BORDER-BOTTOM: solid 1px pink; BORDER-LEFT: solid 1px pink; BORDER-RIGHT: solid 1px pink; BORDER-TOP: solid 1px pink;margin:-1pt 0pt 0pt 0pt '></input>";
	else
...
function cellBlur(event)
{
    ...
    else
    {
      ...
      Input.up().textContent = Input.value;
      Input.style.display = 'none';
    }
}
